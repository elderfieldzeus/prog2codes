...
PropAList readTitleDeed(char filename[], char mode[]) {
	...
	PropAList new = {.count = 0, .max = 22};

	new.lot = (TitleDeed*) malloc(sizeof(TitleDeed) * new.max);

	FILE *fptr = fopen(filename, mode);

	while(fread(new.lot + new.count++, sizeof(TitleDeed), 1, fptr) != 0 && new.count < new.max) {}
