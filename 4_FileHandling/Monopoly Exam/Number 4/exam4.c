...
bool storeProperty(GameBoard board, int player_id) {
	char filename[24];
	sprintf(filename, "player%d_titledeefs.cis", player_id);

	FILE *fptr = fopen(filename, "wb+");

	int playerIndex;

	for(playerIndex = 0; playerIndex < board.playerCount && board.players[playerIndex].id != player_id; playerIndex++) {}

	if(playerIndex == board.playerCount) {
		return false;
	}

	SortedPropLList trav;

	for(trav = board.players[playerIndex].owned; trav != NULL; trav = trav->next) {
		fwrite(&trav->lot, sizeof(TitleDeed), 1, fptr);
	}
